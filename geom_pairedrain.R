...
geom_pairedrain <- function(mapping = NULL,
                      data = NULL,
                      condition_by = NULL,
                      trim = TRUE,
                      show.legend = NA,
...
)
{
  ...
  if (is.null(data) | is.null(condition_by)){
    stop("ERROR: You must specify a data and condition by arguement to use this function \n
         example:\n geom_pairedrain(data = x, condition_by = 'time')")
  }
  
  if (sum(colnames(data) %in% condition_by) != 1){
    stop("ERROR: the condition_by arguement must be a column in your dataframe")
  }
  
  if (length(unique(data[,condition_by])) != 2){
    stop("ERROR: This function currently only supports two conditions")
  }
  
  
  
  e1.1 <- rlang::exec(geom_point_sorted, data = dplyr::filter(data, !!rlang::sym(condition_by) == unique(data[,condition_by][1])),
                    inherit.aes = TRUE, !!!point.args) # bang, bang, bang
  e1.2 <- rlang::exec(geom_point_sorted, data = dplyr::filter(data, !!rlang::sym(condition_by) == unique(data[,condition_by][2])),
                    inherit.aes = TRUE, !!!point.args)
  
  e3.1 <- rlang::exec(gghalves::geom_half_boxplot, #data = dplyr::filter(data, !!rlang::sym(condition_by) == unique(data[,condition_by][1])),
                      inherit.aes = TRUE, !!!boxplot.args)
  # e3.2 <- rlang::exec(gghalves::geom_half_boxplot, data = dplyr::filter(data, !!rlang::sym(condition_by) == unique(data[,condition_by][2])),
  #                     inherit.aes = TRUE, !!!boxplot.args)
  
  e4.1 <- rlang::exec(gghalves::geom_half_violin, #data = dplyr::filter(data, !!rlang::sym(condition_by) == unique(data[,condition_by][1])),
                      inherit.aes = TRUE, !!!violin.args)
  # e4.2 <- rlang::exec(gghalves::geom_half_violin, data = dplyr::filter(data, !!rlang::sym(condition_by) == unique(data[,condition_by][2])),
  #                     inherit.aes = TRUE, !!!violin.args)
  ...
  if (!is.null(id.long.var)){
    ...
    # list(e4.1, e4.2, e3.1, e3.2, e1.1, e1.2)
    list(e4.1, e3.1, e1.1, e1.2)
