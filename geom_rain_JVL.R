...
)
{
  ...
  if (!is.null(id.long.var)){
    ...
    data_ordered = function(data){
      data <-
        data |>
        dplyr::arrange(!!rlang::sym(id.long.var), data %>% arrange(across(names(data)[2]))) # TIME IS HARDCODED #Jordy: !!dplyr::arrange(data, across(names(data)[2])) /// Tested & sworks I think?? -> temp %>% arrange(across(names(temp)[2]))
      data
    }
