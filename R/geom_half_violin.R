...
GeomHalfViolin <- ggproto(
  ...
  draw_group = function(self, data, side = "l", nudge = 0, ..., quantiles = NULL) {
    ...
    if (!is.null(quantiles) & !scales::zero_range(range(data$y))) {
      stopifnot(all(quantiles >= 0), all(quantiles <= 1))
      ...
      ggplot2:::ggname("geom_half_violin", grid::grobTree(
