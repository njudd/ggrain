...
pacman::p_load(ggplot, rlang, grid, ggpp, lavaan, gghalves, patchwork, remotes, DT, ggsignif)
...
<br>
Now we add a significance test between the two timepoints using the package ggsignif for young participants!

```{r rainclouds aging two timepoints sig inference, echo = T}
...
bounds <- function(x){
  ...
  l_unique_min1 <- length(unique(likert_data$pre))-1
  vec <- as.numeric(rep(0, l_unique_min1))
  proportions <- as.numeric(table(likert_data$pre)/length(likert_data$pre))
