...
Here we plot the species overlapping in each time point.
...
We can easily flank them using the `rain.side` argument for 2-by-2 flanking (i.e., `'f2x2'`). This also automatically uses `ggpp::position_dodgenudge` to dodge the boxplots.
...
We can connect each plant with lines using the `id.long.var` argument. As with the `cov` argument it must be a string linking the id's of each observation across time.
...
####### left off here, you need to describe the flanking option
###### probably at the first flank



# add colors to the dots and lines in the above graph
# than end with a stat summary line
